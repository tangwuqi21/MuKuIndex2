...
public class OrderDelivemiddleServiceImpl extends ServiceImpl<OrderDelivemiddleMapper, OrderDelivemiddle> implements IOrderDelivemiddleService {
    ...
    public ResponseEnvelope searchOrderDelivemiddleListPage(OrderDelivemiddleQuery dto) {
        ...
        if (!StringUtils.isEmpty(dto.getSupplierName())) {
            List<Long> supplierIds = (List<Long>) assetServiceFeign.getIdsBySupplierName(dto.getSupplierName(), TokenUtil.getToken());
        ...
        List<OrderDelivemiddleVO> orderDelivemiddleVOList = resultList.stream().map(a -> {
            ...
            // 5.查询供应商名称
            Customer customer = assetServiceFeign.searchCustomerOne(orderDeliverecord.getSupplierId(), TokenUtil.getToken()).getData();
    ...
    public ResponseEnvelope uploadFileCheck(MultipartFile file, Long moduleId) throws Exception {
        ...
        AssetTmplInfo assetTmplInfo = assetServiceFeign.searchAssetTmplInfoOne(moduleId, TokenUtil.getToken()).getData();
        String fileUrl = null;
        // 创建excel工作簿对象
